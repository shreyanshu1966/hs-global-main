 ...
 *   - Products  → Product.seo.{metaTitle, metaDescription, h1Tag, keywords[], ogImage, canonicalUrl}
 *                 (+ optional slug change from the "New Url" column, with a 301 Redirect)
 *   - Categories→ Category.seo.{...}
 *   - Static    → PageSeo collection (keyed by path)
 *   - Slug moves→ Redirect collection (from → to, 301)
 *
 * SAFE BY DEFAULT: runs a dry-run and prints a report. Nothing is written
 * unless you pass --apply. Take a `mongodump` backup before --apply.
 *
 * Usage:
 *   node scripts/import-seo-sheet.js                 # dry-run report
 *   node scripts/import-seo-sheet.js --apply         # write changes (incl. slug moves + redirects)
 *   node scripts/import-seo-sheet.js --apply --no-slug   # write SEO copy only, keep slugs
 *   node scripts/import-seo-sheet.js --csv "/path/to/sheet.csv"
 */
...
async function main() {
  ...
  for (const row of dataRows) {
    ...
    if (curPath.startsWith('/product/')) {
      ...
      const product = await Product.findOne({ productId: curSlug });
      if (!product) {
      ...
      if (DO_SLUGS && newPath !== curPath && newPath.startsWith('/product/')) {
        ...
        if (newSlug && newSlug !== curSlug && existingIds.has(newSlug)) {
          ...
          newSlug = null; // keep old slug; still import SEO
        }
      }
      if (newSlug) canonicalPath = `/product/${newSlug}`;

      const seo = buildSeo(row, canonicalPath);
      product.seo = Object.assign({}, product.seo ? product.seo.toObject?.() || product.seo : {}, seo);

      if (newSlug) {
        product.seo.slug = newSlug;
        product.productId = newSlug;
