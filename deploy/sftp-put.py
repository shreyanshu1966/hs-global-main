#!/usr/bin/env python3
"""
Uploads a single file to the VPS over SFTP (binary-safe, no PTY line
limits). Companion to ssh-run.py / sftp-get.py.

Usage:
  python deploy/sftp-put.py <host> <user> <local_path> <remote_path>
...
def main():
    if len(sys.argv) < 5:
        sys.stderr.write("Usage: python sftp-put.py <host> <user> <local_path> <remote_path>\n")
        sys.exit(1)
    ...
    try:
        ...
        sys.stderr.write(f"SFTP upload failed: {e}\n")
        sys.exit(1)
    finally:
        client.close()
