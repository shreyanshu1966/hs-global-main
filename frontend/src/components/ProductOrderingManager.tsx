...
const ProductOrderingManager: React.FC<ProductOrderingManagerProps> = ({ onBack }) => {
  ...
  // Cross-category mode: category=null with a subcategory set
  const isCrossMode = scope.category === null && scope.subcategory !== null;

  // Drag state — products
  const dragIdx = useRef<number | null>(null);
  // Drag state — category order
    ...
    try {
      const res = await fetch(`${API_BASE}/admin/product-ordering`, {
        ...
        body: JSON.stringify({
          ...
          categoryOrder: isCrossMode ? categoryOrder : [],
        }),
      });
      const data = await res.json();
      if (data.success) {
  ...
  // ── Add / Remove categories (cross-category mode) ───────────────────────────
  const addCategoryToOrder = (cat: string) => {
    setCategoryOrder(prev => [...prev, cat]);
    setIsDirty(true);
  };
  const removeCategoryFromOrder = (cat: string) => {
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors border ${
              scope.category === null && scope.subcategory === null
          ...
          {/* Cross-category subcategory (only shown when no category is selected) */}
          {scope.category === null && (
      ...
      {loading ? (
        ...
        <>
          {/* ── Cross-category: Category Group Order ────────────────────────── */}
          {isCrossMode && (
            <div className="bg-white rounded-2xl border border-orange-100 shadow-sm overflow-hidden">
              <div className="px-5 py-3.5 border-b border-orange-50 flex items-center gap-2">
                <Layers className="w-4 h-4 text-orange-500" />
                <div>
                  <h3 className="text-sm font-semibold text-gray-900">Category Group Order</h3>
                  <p className="text-xs text-gray-400 mt-0.5">
                    Drag categories to set which group appears first on the "{scope.subcategory}" page.
                    Within each group, the per-category subcategory ordering is used.
                  </p>
                </div>
              </div>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 divide-y lg:divide-y-0 lg:divide-x divide-gray-50">
                ...
                <div>
                  <div className="px-5 py-2.5 border-b border-gray-50 flex items-center justify-between">
                    <span className="text-xs font-medium text-gray-500">Group sequence</span>
                  ...
                  <div className="divide-y divide-gray-50 min-h-[80px]">
                    {categoryOrder.length === 0 ? (
                      ...
                      categoryOrder.map((cat, idx) => (
                        ...
                        >
                          <GripVertical className="w-4 h-4 text-gray-300 group-hover:text-orange-400 flex-shrink-0" />
                          <span className="text-xs font-mono text-gray-300 w-5 flex-shrink-0">{idx + 1}</span>
                          <span className="flex-1 text-sm font-medium text-gray-800 capitalize">{cat}</span>
                          <button
                            onClick={() => removeCategoryFromOrder(cat)}
                ...
                <div>
                  <div className="px-5 py-2.5 border-b border-gray-50 flex items-center justify-between">
                    <span className="text-xs font-medium text-gray-500">Available categories</span>
                    <span className="text-xs bg-gray-100 text-gray-500 font-semibold px-2 py-0.5 rounded-full">
                      {unorderedCategories.length}
                    </span>
                  </div>
                  <div className="divide-y divide-gray-50 min-h-[80px]">
                    {unorderedCategories.length === 0 ? (
                      <p className="text-xs text-gray-400 text-center py-6">All categories added.</p>
                    ) : (
                      unorderedCategories.map(cat => (
                        <div
                          key={cat}
                          className="flex items-center gap-3 px-4 py-2.5 hover:bg-gray-50 transition-colors group"
                        >
                          <span className="flex-1 text-sm font-medium text-gray-700 capitalize">{cat}</span>
                          <button
                            onClick={() => addCategoryToOrder(cat)}
